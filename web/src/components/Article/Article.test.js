...
describe('Article', () => {
  ...
  it('renders a full blog post if summary is explicitly excluded', () => {
    render(<Article article={ARTICLE} summary={false} />)
    const ellipsis = screen.queryByText('...')

    expect(ellipsis).not.toBeInTheDocument()
    expect(screen.getByText(ARTICLE.title)).toBeInTheDocument()
    // The full (untruncated) article body should be present
    expect(screen.getByText(ARTICLE.body, {exact: true})).toBeInTheDocument()
  })
